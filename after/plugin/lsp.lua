vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
        ...
        vim.keymap.set('n', '<leader>cdw', require('telescope.builtin').diagnostics, { desc = "[W]orkspace" })
        vim.keymap.set('n', '<leader>cdb', function() return require('telescope.builtin').diagnostics({ bufnr = 0 }) end,
            { desc = "[B]uffer" })
        vim.keymap.set('n', '<leader>cde',
            function() return require('telescope.builtin').diagnostics({ bufnr = 0, severity = "Error" }) end,
            { desc = "[B]uffer Errors" })
        vim.keymap.set('n', '<leader>cdo', vim.diagnostic.open_float, { desc = '[O]pen Diagnostic Float' })
